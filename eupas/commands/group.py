...
class Command(ScrapyCommand):
    ...
    def process_options(self, args, opts):
        ...
        if not (self.input_path.exists() and self.input_path.is_file()):
            raise UsageError("Invalid -i value, use a valid path to a file", print_help=False)
        if self.input_path.suffix != '.json':
            raise UsageError("Invalid -i value, json file expected", print_help=False)

        self.output_folder = Path(opts.output or "")
        if self.output_folder.exists() and not self.output_folder.is_dir():
            raise UsageError("Invalid -i value, use a valid path to a folder", print_help=False)
        ...
        try:
            ...
            raise UsageError("Invalid -c value, use a valid float between 0 and 1", print_help=False) from e
    ...
    def run(self, args, opts):
        ...
        if not set(args).issubset(set(Study.fields)):
            raise UsageError("At least one -g value isn't a valid field name", print_help=False)
        ...
        for i, field_name in enumerate(args):
            ...
            if self.method == 1:
                values = {
                    firstShowTuple((self.serialize(study[field_name]), str(study[field_name])))
                ...
                while len(values) > 1:
                    val = values.pop()
                    matches = get_close_matches(val, values, n=len(values), cutoff=self.cutoff)
                    values.difference_update(matches)
                    if self.include_own_group_name: matches.append(val)
                    groups.setdefault(val[1], sorted([m[1] for m in matches]))
                else:
                    val = values.pop()
                    groups.setdefault(val[1], [val[1]] if self.include_own_group_name else [])
            ...
            for group_name, field_names in sorted(groups.items()):
                ...
                field_cells = [WriteOnlyCell(value=name, ws=sheet) for name in field_names]
                sheet.append([group_cell, field_cells[0] if field_cells else ''])
