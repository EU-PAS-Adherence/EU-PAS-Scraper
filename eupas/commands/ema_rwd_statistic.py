...
class Command(PandasCommand):
    ...
    # Mostly used to fill NA values with a string to work with methods requiring non NA values
    temp_na_name = '$NA'
    ...
    def preprocess(self, df):
        ...
        self.logger.info('Transforming funding field')
        df[self.funding_field_name] = np.where(
            df['funding_sources'].eq('No external funding'),
            df[self.funding_field_name].fillna(NO_FUNDING),
        ...
        df[self.funding_field_name] = np.where(
            ...
            df[self.funding_field_name].fillna(EMA)
            .str.split('; ').apply(
                lambda x: list(set(x).union(set([EMA])))
            ),
            np.where(
                df[self.funding_field_name].notna(),
                df[self.funding_field_name].fillna(self.temp_na_name)
                .str.split('; ').apply(set).apply(list),
                np.nan
            )
        )
        ...
        # NOTE: The following correction is not needed (every number_of_subjects of a study is grouped correctly later)
    ...
    def create_variables(self, df):
        ...
        funding_map = {
            # NOTE: These values can not be grouped and have to be mapped manually
            # NOTE: These values should be overriden by funding_sources_grouped_override if a manual mappping is desired
            # NOTE: Uncomment to map these to 'Unclear' instead of pd.NA
            # 'EMA; Other': 'Unclear',
        ...
        variables = variables.assign(
            ...
            multiple_funding_sources=df['multiple_funding_sources_override'].combine_first(
                df[self.funding_field_name].apply(
            ...
            studied_medical_conditions=(
                df['medical_conditions'].notna()
                | df['additional_medical_conditions'].notna()
            ),
    ...
    def create_grouped_agg(self, df):
        ...
        grouped = grouped.fillna(self.temp_na_name).explode(self.funding_field_name) \
            .groupby(by=self.funding_field_name, dropna=False)
    ...
    def encode_variables(self, df, drop_references=True):
        ...
        dummy_with_na_drop_map = {
            ...
            # NOTE: Combined Categories => Many categories: Use funding_sources_grouped instead
            # 'funding_sources': 'Pharmaceutical company and other private sector ',
            # NOTE: Combined Categories => Many categories: Dropped
    ...
    def run(self, args, opts):
        ...
        self.logger.info('Adding extra columns if unspecified...')
            ...
            with pd.ExcelWriter(self.output_folder / f'{self.input_path.stem}_statistics_variables_frequencies{suffix}.xlsx', engine='openpyxl') as writer:
                ...
                for col in sorted(df.columns):
                    ...
                    def calculate_and_write_frequencies(df, col, writer, dropna=True, col_offset=0):
                        ...
                        if col in self.variables_category_array_fields:
                    ...
                    if df.loc[:, [col]].isna().any().loc[col]:
                        ...
                        calculate_and_write_frequencies(
                            ...
                            col_offset=8 if col in self.variables_category_array_fields else 4
                        )
        ...
        data.loc[:, [
            ...
            'has_protocol', 'has_result', 'risk_management_plan'
        ]].rename(
